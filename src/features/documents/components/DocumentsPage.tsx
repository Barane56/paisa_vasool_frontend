...
const FileQueueItem = ({
  ...
  const statusIcon =
    ...
    doc.status === 'extracted'                ? <CheckCircle2 size={15} className="text-green-500" /> :
    doc.status === 'failed'                   ? <AlertCircle size={15} className="text-red-400" /> : null;
  ...
  return (
    <div className="rounded-xl border border-surface-100 bg-white overflow-hidden transition-all">
      ...
      {job.error && (
        <div className="px-3 pb-3">
          <p className="text-xs text-red-500 bg-red-50 rounded-lg px-3 py-2">{job.error}</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {email.failure_reason && (
        <div className="rounded-xl bg-red-50 border border-red-100 px-4 py-3 flex items-start gap-2">
          ...
          <div>
            <p className="text-xs font-semibold text-red-700 mb-0.5">Processing note</p>
            <p className="text-xs text-red-600">{email.failure_reason}</p>
          </div>
        </div>
      )}
    </div>
